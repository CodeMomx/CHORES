module.exports = function (app, passport, db, stripe) {
  ...
  app.get("/profile", isLoggedIn, function (req, res) {
        let findParameters = {}; 
    if (req.user.local.role === 'parent') {
      findParameters = {
        parent: req.user.local.email,
      };
    } else if (req.user.local.role === 'child') {
      findParameters = {
  ...
  app.delete("/clearChores", (req, res) => {
    db.collection("completed").deleteMany(
      { },
      (err, result) => {
        if (err) return res.send(500, err);
        res.send("Message deleted!");
  ...
  app.post("/create-payment-intent", async (req, res) => {
    db.collection("completed")
      .find({ parent: req.user.local.email })
